...
import { createClient } from '@remixproject/plugin-vscode';
import { CompilerApiMixin } from './lib/api/compiler-api';
import { ICompilerApi } from '@remix-project/remix-lib';
import { CompileTabLogic } from './lib/logic';
...
export class CompilerClientApi extends CompilerApiMixin(PluginClient) implements ICompilerApi {
  ...
  constructor() {
    super();
    this.client = createClient(this as any);
  ...
  async getAppParameter(name) {
    return await this.client.call('config', 'getAppParameter', name);
  }

  async setAppParameter(name, value) {
    await this.client.call('config', 'setAppParameter', name, value);
  }
