...
export const bleach: any = {
  matcher: /<\/?([a-zA-Z0-9]+)*(.*?)\/?>/gim,

  whitelist: ['a', 'b', 'p', 'em', 'strong'],

  analyze: function (html: any) {
    html = String(html) || '';
  ...
  sanitize: function (html: any, options = { mode: 'white', list: bleach.whitelist, encode_entities: false }) {
    html = String(html) || '';
    ...
    matches.forEach(function (tag: any) {
      if (mode === 'white') {
