...
export const extractNameFromKey = (key: string): any => {
  if (!key) return;
...
export const extractParentFromKey = (key: string): any => {
  if (!key) return;
  const keyPath = key.split('/');
...
export const createNonClashingNameAsync = async (name: string, fileManager: any, prefix = '') => {
...
export const createNonClashingTitle = async (name: string, fileManager: any) => {
  if (!name) name = 'Undefined';
  let _counter;
  let exist = true;
...
export const joinPath = (...paths: any) => {
  paths = paths.filter((value: any) => value !== '').map((path: any) => path.replace(/^\/|\/$/g, '')); // remove first and last slash)
...
export const shortenAddress = (address: any, etherBalance?: any) => {
  const len = address.length;

  return address.slice(0, 5) + '...' + address.slice(len - 5, len) + (etherBalance ? ' (' + etherBalance.toString() + ' ether)' : '');
};

export const addressToString = (address: any) => {
...
export const is0XPrefixed = (value: any) => {
  return value.substr(0, 2) === '0x';
};

export const isHexadecimal = (value: any) => {
  return /^[0-9a-fA-F]+$/.test(value) && value.length % 2 === 0;
};

export const isValidHash = (hash: any) => {
...
export const shortenHexData = (data: any) => {
  if (!data) return '';
