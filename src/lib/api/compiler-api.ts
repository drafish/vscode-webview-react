...
import { CompileTabLogic, parseContracts } from '../logic'; // eslint-disable-line
import type { ConfigurationSettings } from '@remix-project/remix-lib';

export const CompilerApiMixin = (Base) =>
  class extends Base {
    currentFile: string;
    compilationDetails: {
      contractMap:
    ...
    async compileWithParameters(compilationTargets, settings) {
      ...
      const res = await compile(compilationTargets, settings, (url, cb) =>
    ...
    async visitsContractApi(source, data): Promise<{ contractMap: { file: string } | Record<string, any>; contractsDetails: Record<string, any>; target?: string }> {
      return new Promise((resolve) => {
        ...
        this.compiler.visitContracts((contract) => {
          ...
          contractsDetails[contract.name] = parseContracts(contract.name, contract.object, this.compiler.getSource(contract.file));
        });
        return resolve({
          contractMap,
          contractsDetails,
